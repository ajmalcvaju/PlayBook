...
export const UserRepositoryImpl: UserRepository = {
  ...
  async confirmBooking(id:string,userId:string|null,turfId:string):Promise<UpdateResult>{
    ...
    const newBooking = new BookingModel({
      slotId: id,turfId: turfId,userId: userId,paid: price});
  ...
  async updateRatings(review: Review): Promise<Review> {
    ...
    const [ratingData] = await ReviewModel.aggregate([
      ...
      {$group: {_id: "$turfId",averageRating: { $avg: "$rating" },totalReviews: { $sum: 1 }}}]);
    const averageRating = ratingData?.averageRating || 0;
    const totalReviews = ratingData?.totalReviews || 0;
      await TurfModel.findOneAndUpdate(
      {_id: new ObjectId(review.turfId)},{rating:averageRating,votes:totalReviews},{ new: true, upsert: true });
    return updatedReview.toObject();
  },
  async getReviews(turfId: string): Promise<[Review[], number, number]> {
    const ObjectId = mongoose.Types.ObjectId;
    const reviews = await ReviewModel.find({ turfId }).populate('turfId', 'turfName').populate('userId', 'firstName lastName').exec();
    const [ratingData] = await ReviewModel.aggregate([
      { $match: { turfId: new ObjectId(turfId) } },
      {$group: {_id: "$turfId",averageRating: { $avg: "$rating" },totalReviews: { $sum: 1 }}}]);
      const averageRating = ratingData?.averageRating ? parseFloat(ratingData.averageRating.toFixed(2)): 0;
    const totalReviews = ratingData?.totalReviews || 0;
    return [reviews,averageRating,totalReviews];
  } ,
  async report(turfId:string,userId:string,issue:string): Promise<void>{
    const report = await ReportModel.findOneAndUpdate(
      { turfId, userId },{ issue },{ upsert: true, new: true }
    );
    return
  },
