...
const ForgotTurfPassword = () => {
  ...
  const submit = handleSubmit(async (data) => {
    ...
    try {
      const response = await apiClient.post("/turfs/forgot-password", data, {
        headers: { "Content-Type": "application/json" },
      });
      if (response.status === 200) {
        localStorage.setItem("turfEmail",data.email as string);
        setEmailModal(false);
        setOtpModal(true);
      } else {
  ...
  const OtpSubmit = handleSubmit(async (data) => {
    ...
    try {
      ...
      data.email = email;
      const response = await apiClient.post("/turfs/verify-otp-forgotpassword", data, {
        headers: { "Content-Type": "application/json" },
      });
      if (response.status === 200) {
        setOtpModal(false);
      ...
      console.log(error);
      setErrorMessage("Verify your OTP and try again.");
    } finally {
      setIsLoading(false);
    }
  });
  const passwordSubmit = handleSubmit(async (data) => {
    ...
    try {
        const email = localStorage.getItem("turfEmail");
        data.email = email;
      const response = await apiClient.post("/turfs/change-forgotpassword", data, {
        headers: { "Content-Type": "application/json" },
      });
      if (response.status === 200) {
          console.log("hi");
          console.log(response);
          localStorage.setItem("turfToken", response.data.accessToken);
          localStorage.setItem("accessToken", response.data.accessToken);
          dispatch(signInSuccess(response.data.turf));
...
export default ForgotTurfPassword;
